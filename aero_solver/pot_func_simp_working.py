...
class aero_solver_osc_flat:
    ...
    def V_ind_ub_field(self, x1_N, y1_N, x2_N, y2_N, Gamma_N,dd):
        ...
        v_core = 1.3*np.tile(v_core, (len(x1_N), 1))

        # Calculate induced velocities using broadcasting
        u_ind_p =  dy*( 1 / (2 * np.pi * (r_squared**2 + v_core**4)**0.5)) * Gamma_N # Shape: (len(x1_N), len(x2_N))
        v_ind_p =  dx*(-1 / (2 * np.pi * (r_squared**2 + v_core**4)**0.5)) * Gamma_N # Shape: (len(y1_N), len(y2_N))
