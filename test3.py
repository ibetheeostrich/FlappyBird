...
import aero_solver.aero_objects_3 as ao
...
alpha_dot = lambda t: 2*np.pi*np.deg2rad(10)*np.sin(2*np.pi*t) if t < 0.5 else 0.0

u = lambda t: 5.0*t
h = lambda t: 0.0#1-np.cos(2*np.pi*t)
alpha = lambda t: np.deg2rad(10) - np.deg2rad(10)*np.cos(2*np.pi*t) if t < 0.5 else np.deg2rad(20)
...
#####################################################################################    
        ...
        if round(t/t_step) % 5== 0:
            ...
            gb = np.pi * be.c(t) * be.x_dot(t) * (be.fourier[0] + be.fourier[1] * 0.5) + np.sum(np.concatenate((field.tev, field.lev, field.ext)))           
            # print(f'{t:.2f} {abs(be.fourier[0]) - lesp_crit:.3f} {gb}') 
            # print(be.fourier - be.update_fourier_test(
            #      np.concatenate((field.tev_x, field.lev_x, field.ext_x)),
            #      np.concatenate((field.tev_y, field.lev_y, field.ext_y)),
            #      np.concatenate((field.tev, field.lev, field.ext)), t
            #      ))
        # print(x_dot(t))
